...
public class UserController {
    ...
    public String registerUser(User user, Model model) {
        String password_pattern = "(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])";
        Pattern pattern = Pattern.compile(password_pattern);
        Matcher matcher = pattern.matcher(user.getPassword().trim());
        //commenting this because my pattern is not working but the solution should be like this
       /* if (!matcher.matches()) {
            model.addAttribute("User", user);
